...
namespace EODAddIn.BL
{
    ...
    public class UDF
    {
        ...
        public double? EOD_GetOpen(string ticker, DateTime date)
        {
            try
            {
                List<Model.EndOfDay> res = HistoricalAPI.HistoricalAPI.GetEOD(ticker, date, date, "d");
                return res[0].Open;
            }
        ...
        public double? Intraday_GetOpen(string ticker, DateTime date)
        {
            try
            {
                List<Model.Intraday> res = IntradayAPI.IntradayAPI.GetIntraday(ticker, date, date, "1m");
                return res[0].Open;
            }
            catch (APIException ex)
            {
